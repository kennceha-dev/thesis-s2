...
Kerangka pikir penelitian ini dimulai dari pengamatan bahwa dokumen transaksi berbahasa Indonesia tidak selalu dapat diproses secara stabil dengan satu jalur ekstraksi yang sama. Receipt, invoice, kuitansi, dan laporan keuangan sederhana dapat memiliki variasi layout, kualitas gambar, format angka lokal, singkatan, stempel, tanda tangan, serta catatan tulisan tangan. Pada sebagian field, hasil OCR dan rule sederhana sudah cukup untuk menghasilkan nilai yang benar. Namun pada field lain, hasil OCR dapat terlihat valid secara format tetapi sebenarnya salah, ambigu, atau kehilangan karakter penting. Jika seluruh field diproses dengan OCR-only, risiko salah terima menjadi tinggi. Sebaliknya, jika seluruh dokumen selalu dikirim ke LLM atau VLM, biaya komputasi dan latency menjadi lebih besar daripada yang diperlukan.

Berdasarkan masalah tersebut, penelitian ini menempatkan confidence pada level field sebagai variabel utama dalam routing ekstraksi. Gagasan utamanya adalah bahwa kebutuhan pemrosesan tidak ditentukan oleh dokumen secara keseluruhan, tetapi oleh kondisi masing-masing field. Field yang memiliki OCR confidence tinggi, label yang jelas, posisi yang sesuai, format yang valid, dan konsisten dengan field lain dapat diterima melalui jalur ringan. Field yang masih ambigu secara teks dapat dikirim ke text-only LLM. Field yang dicurigai mengalami kerusakan visual atau kesalahan OCR dapat dikirim ke VLM dengan crop gambar. Field yang tetap berisiko setelah proses tersebut ditandai untuk human review.

Dengan gagasan tersebut, confidence tidak diperlakukan sebagai skor tunggal hasil OCR, melainkan sebagai gabungan beberapa bukti. Bukti tersebut mencakup kualitas OCR, kecocokan label, kedekatan posisi, validasi format, konsistensi konteks, dan kesepakatan antar field candidate. Skor ini digunakan untuk menjawab pertanyaan praktis: apakah nilai field cukup aman diterima otomatis, perlu diperiksa ulang oleh model berbasis teks, perlu dibaca dari gambar, atau harus ditahan untuk review manusia. Karena itu, routing dalam penelitian ini berfungsi sebagai mekanisme untuk memilih tingkat pemrosesan yang sesuai dengan risiko tiap field.

Alur sistem yang diusulkan dimulai dari dokumen input, kemudian dilakukan OCR dan layout analysis untuk memperoleh teks, bounding box, serta confidence awal. Hasil tersebut digunakan untuk membentuk field candidate melalui label alias, spatial proximity, pola format, posisi tabel, dan kemiripan semantik. Setiap field candidate kemudian diberi confidence score. Jika confidence melewati ambang accept dan validasi kritis terpenuhi, nilai diterima sebagai output. Jika tidak, sistem menentukan apakah ketidakpastian lebih bersifat tekstual atau visual. Ketidakpastian tekstual diarahkan ke text-only LLM, sedangkan ketidakpastian visual diarahkan ke VLM crop. Setelah eskalasi selesai, hasil divalidasi ulang sebelum diterima atau ditandai sebagai human review.
...
![Gambar 3.2 Langkah Penelitian](figures/gambar_3_2_langkah_penelitian.png)
